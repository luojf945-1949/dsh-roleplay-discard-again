...
test('imports an MVU+lore card, creates an Actor session and commits atomically', async () => {
  ...
  try {
    ...
    assert.equal(fullCommitSchema.properties.effects.items.additionalProperties, false)
    assert.deepEqual(fullCommitSchema.properties.effects.items.required, ['kind', 'namespace', 'expectedRevision', 'payload'])
