 ...
 *
 * 与 rp-message-actions、rp-session、rp-conversation-summary 测试目录下的同名文件
 * 内容相同：仓库规定插件之间不得通过相对路径读取彼此的内部文件，这段探测又不足以
 * 抽成独立包，因此各自保留一份。
 */
...
function detectsAssistantReplaceSupport() {
  counter += 1
  const session = Session.create(SessionId(`rp-standard-assistant-replace-probe-${counter}`))
