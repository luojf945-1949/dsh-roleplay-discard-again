window.__ModuleLoader__.load({
	id: "dsh-roleplay-rp-reply-options",
	factory: (require) => {
		var module = { exports: {} };
...
get: (_target, key) => {
		...
		*/
		...
		const REPLY_OPTIONS_ANCHOR_KIND = "rp-reply-options-anchor";
		const REPLY_OPTIONS_RETRACTION_KIND = "rp-reply-options-retraction";
		const latestAnchorCache = /* @__PURE__ */ new WeakMap();
		/** Match one Turn lifecycle that can end in a committed Roleplay reply. */
		function replyOptionsAnchorMatch(event) {
			...
			if (event.type === "tool/result" || event.type === "turn/end") return {
				id: String(turn),
				role: "update"
			};
			return null;
		}
		function replyOptionsAnchorStart(event) {
			return {
		...
		function replyOptionsAnchorUpdate(state, event) {
			if (event.type === "tool/result" && successfulCommitResult(event)) {
				const stored = decodeStoredReplyOptions(event.data?.meta?.extensions?.[REPLY_OPTIONS_EXTENSION_NAMESPACE]);
				return {
					...state,
					committed: true,
					assistantSeq: Number.isSafeInteger(event.data?.meta?.assistant?.seq) ? event.data.meta.assistant.seq : void 0,
					options: stored?.options
				};
			}
			if (event.type === "turn/end") return {
				...state,
				successful: event.data?.reason?.kind === "completed" && state.committed === true && Number.isSafeInteger(state.assistantSeq) && Array.isArray(state.options),
				endSeq: event.seq
			};
			return state;
		}
		const replyOptionsAnchorNodeDefinition = {
			...
			buildViewNode: (context) => {
				...
				return {
					...
					id: context.id,
					target: "chat",
					anchorSeq: state.assistantSeq + .12,
		...
		function replyOptionsRetractionMatch(event) {
			return messageAction(event) !== void 0 ? {
				id: String(event.seq),
				role: "start"
			} : null;
		}
		function replyOptionsRetractionStart(event) {
		...
		const replyOptionsRetractionNodeDefinition = {
			...
			buildViewNode: (context) => !Number.isSafeInteger(context.state?.seq) ? null : {
				...
				id: context.id,
				target: "chat",
				anchorSeq: context.state.seq + .115,
				location: { kind: "session" },
				visibility: "visible",
				data: context.state
			}
		...
		function latestReplyOptionsAnchorKey(chat) {
			...
			for (const key of order) {
				...
				if (node?.kind === "rp-reply-options-anchor") {
					anchors.push(node);
					continue;
				}
				if (node?.kind === "rp-reply-options-retraction") {
		...
		const css = {
			"card": "rp-reply-options-card",
			"error": "rp-reply-options-error",
		...
		function ReplyOptionsCard({ options, sessionId, disabled = false, sendReply, openSettings }) {
			...
			return h(MotionConfig, { reducedMotion: "user" }, h(LazyMotion, { features: domAnimation }, h(m.section, {
				className: css.card,
				...
				disabled: busy,
				"data-sending": sendingIndex === index ? "true" : void 0,
				"aria-busy": sendingIndex === index ? "true" : void 0,
				onClick: () => void choose(option, index),
				whileTap: busy ? void 0 : motion.whileTap
			}, h("span", {
				className: css.optionNumber,
				"aria-hidden": true
		...
		function createReplySender(sessions) {
			return async (sessionId, text) => {
				...
				if (conversation === void 0) throw new Error("reply options could not resolve the scoped conversation");
				await conversation.send(text);
			};
		}
		function replyOptionsMotion(reducedMotion) {
			return {
				initial: {
					opacity: 0,
					y: reducedMotion ? 0 : 4
		...
		//#endregion
		exports.ReplyOptionsAnchor = ReplyOptionsAnchor;
		exports.ReplyOptionsCard = ReplyOptionsCard;
		exports.ReplyOptionsRetraction = ReplyOptionsRetraction;
		exports.apply = apply;
		exports.createReplySender = createReplySender;
		exports.inject = inject;
		exports.replyOptionsMotion = replyOptionsMotion;
