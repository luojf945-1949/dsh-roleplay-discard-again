...
export function messageActionError(reason) {
  ...
  // 当前 DSH 基线不支持改写助手消息（见 docs/known-limitations.md LIM-1）：说清结果、原因与恢复方式。
  if (code === 'ASSISTANT_REPLACE_UNAVAILABLE') {
    return '当前版本还不支持改写已经生成的回复，编辑、删除和重新生成暂时无法完成；应用更新后可以恢复。'
  }
