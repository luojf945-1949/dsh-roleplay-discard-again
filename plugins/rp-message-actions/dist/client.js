window.__ModuleLoader__.load({
	id: "dsh-roleplay-rp-message-actions",
	factory: (require) => {
		var module = { exports: {} };
...
get: (_target, key) => {
		...
		*/
		...
		function decodeRpMessageActionEvent(event) {
			const action = (event?.type === "user/message" ? event.data?.source : event?.type === "assistant/message" ? event.data?.message?.source : void 0)?.rpMessageAction;
			if (action === void 0) return void 0;
			if (!record(action) || action.kind !== "rp-agent/message-action" || action.version !== 1 || ![
				"edit",
				"delete",
				"reroll"
			].includes(action.operation) || !Array.isArray(action.targets) || action.targets.length === 0) throw new TypeError("Roleplay message action metadata is invalid");
			const targets = action.targets.map(validateTarget);
			if (new Set(targets.map(rpMessageActionTargetKey)).size !== targets.length) throw new TypeError("Roleplay message action targets must be unique");
		...
		function validateTarget(target) {
			...
			if (target.kind === "turn") {
				if (!Number.isSafeInteger(target.turn) || target.turn < 0) throw new TypeError("Roleplay failed-turn target requires a non-negative turn");
				return {
					kind: "turn",
					turn: target.turn
				};
			}
			if (target.kind !== "message" || target.role !== "user" && target.role !== "assistant" || typeof target.messageId !== "string" || target.messageId.length === 0) throw new TypeError("Roleplay message target is invalid");
			return {
				kind: "message",
				role: target.role,
		...
		function validateReplay(replay) {
			if (!Array.isArray(replay) || replay.length === 0) throw new TypeError("Roleplay reroll metadata requires replay messages");
			const ids = /* @__PURE__ */ new Set();
			for (const message of replay) {
				if (!record(message) || message.role !== "user" || typeof message.id !== "string" || ids.has(message.id) || message.source?.kind !== "user" || !Array.isArray(message.content) || message.content.length === 0 || message.content.some((block) => block?.type !== "text" || typeof block.text !== "string")) throw new TypeError("Roleplay reroll replay message is invalid");
				ids.add(message.id);
			}
		}
		...
		*/
		...
		function messageActionError(reason) {
		...
		function actionError(code) {
			const error = /* @__PURE__ */ new Error("roleplay message action failed");
			error.code = code;
			return error;
		}
		...
		function projectMessageActionDetail(snapshot, target, fallbackNode) {
			...
			const node = nodes.find((candidate) => actionNodeTargetKey(candidate) === safeTargetKey(target)) ?? nodes.find((candidate) => failedAssistantTargetKey(candidate) === safeTargetKey(target));
			if (node === void 0 || node.data?.deleted === true) return null;
			if (node.kind === "rp-floor-opening-actions") return {
			...
			if (common.role === "user") return {
				...common,
				canSaveAndReroll: currentTail && replayable && !sharedAssetMutation && nodeTargetKey === latestUserTargetKey(nodes, turn)
			};
			return {
				...common,
		...
		function projectFailedTurnDetail(snapshot, nodes, target) {
			...
			return {
				...
				canEdit: false,
				canDelete: true,
				canReroll: latestVisibleTurn(nodes) === target.turn && replayable && !sharedAssetMutation,
				canSaveAndReroll: false,
				edited: false,
				failed: true,
				failureKind: turnSurfaceEndReasonKind(state),
		...
		function conversationNodes(chat, fallbackNode) {
			...
			if (fallbackNode === void 0) return nodes;
			return nodes.some((node) => node === fallbackNode || node?.kind === fallbackNode.kind && node?.id === fallbackNode.id) ? nodes : [...nodes, fallbackNode];
		}
		...
		function actionNodeTurn(node) {
			...
			return node?.location?.kind === "turn" || node?.location?.kind === "step" ? node.location.turn.turn : void 0;
		}
		...
		function turnUserNodes(nodes, turn) {
			return nodes.filter((node) => node?.kind === "rp-floor-user-actions" && node.data?.deleted !== true && actionNodeTurn(node) === turn);
		}
		function replayableTurn(nodes, turn) {
			const users = turnUserNodes(nodes, turn);
			return users.length > 0 && users.every((node) => node.data?.hasNonTextContent !== true && typeof node.data?.text === "string" && node.data.text.trim().length > 0);
		}
		function latestUserTargetKey(nodes, turn) {
			let latest;
			for (const node of turnUserNodes(nodes, turn)) if (latest === void 0 || (node.data?.seq ?? -1) >= (latest.data?.seq ?? -1)) latest = node;
		...
		function latestVisibleTurn(nodes) {
			...
			return turns.length === 0 ? void 0 : Math.max(...turns);
		}
		function suffixHasSharedAssetMutation(nodes, selectedTurn) {
			...
			for (const turn of turns) if (roleplayTurnState(nodes, turn)?.sharedAssetMutation === true) return true;
			return false;
		}
		/** Fold a native assistant edit/delete carrier into its original action row. */
		function updateAssistantActionState(state, event) {
		...
		function assistantActionMatch(event) {
			if (event?.type !== "assistant/message" || event.data?.message?.source?.kind !== "model" || isSelectedOpeningMessage(event)) return null;
			const action = decodeRpMessageActionEvent(event);
			if (action !== void 0) {
		...
		const css = {
			"assetNotice": "rp-message-actions-assetNotice",
			"assistantActionHost": "rp-message-actions-assistantActionHost",
			"assistantEffectNodeMarker": "rp-message-actions-assistantEffectNodeMarker",
			"commitFailure": "rp-message-actions-commitFailure",
			"commitToolMarker": "rp-message-actions-commitToolMarker",
			"deleteAction": "rp-message-actions-deleteAction",
			"deleteConfirm": "rp-message-actions-deleteConfirm",
			"deleteSummary": "rp-message-actions-deleteSummary",
			"deletedAssistantMarker": "rp-message-actions-deletedAssistantMarker",
			"deletedOpeningMarker": "rp-message-actions-deletedOpeningMarker",
			"deletedUserMarker": "rp-message-actions-deletedUserMarker",
			"dialog": "rp-message-actions-dialog",
			"editedAssistantMessage": "rp-message-actions-editedAssistantMessage",
			"editedMessagePortalAnchor": "rp-message-actions-editedMessagePortalAnchor",
			"editedUserMessage": "rp-message-actions-editedUserMessage",
			"editorNotice": "rp-message-actions-editorNotice",
			"error": "rp-message-actions-error",
			"failedTurnDeleteAction": "rp-message-actions-failedTurnDeleteAction",
			"failedTurnRecoveryActions": "rp-message-actions-failedTurnRecoveryActions",
			"failedTurnStatus": "rp-message-actions-failedTurnStatus",
			"failedTurnStatusCopy": "rp-message-actions-failedTurnStatusCopy",
			"failedTurnStatusDot": "rp-message-actions-failedTurnStatusDot",
			"floorAction": "rp-message-actions-floorAction",
			"floorActionHost": "rp-message-actions-floorActionHost",
			"floorActions": "rp-message-actions-floorActions",
			"inactiveActionNodeMarker": "rp-message-actions-inactiveActionNodeMarker",
			"inlineEditor": "rp-message-actions-inlineEditor",
			"inlineEditorFooter": "rp-message-actions-inlineEditorFooter",
			"inlineEditorPortalAnchor": "rp-message-actions-inlineEditorPortalAnchor",
		...
		const userFloorNodeDefinition = {
			...
			start: (_context, match) => ({
				...
				target: {
					kind: "message",
					role: "user",
					messageId: userMessageId(match.event)
				},
				edited: false,
				deleted: false
			}),
			update: (context, match) => updateUserActionState(context.state, match.event),
			buildViewNode: (context) => context.state?.turn === void 0 ? null : {
				key: context.key,
				kind: "rp-floor-user-actions",
				id: context.id,
				target: "chat",
		...
		const assistantFloorNodeDefinition = {
			...
			start: (_context, match) => ({
				...
				target: {
					kind: "message",
					role: "assistant",
					messageId: match.event.data.message.id,
					turn: match.event.data.turn,
		...
		const openingFloorNodeDefinition = {
		...
		const suffixActionNodeDefinition = {
			kind: "rp-message-suffix-action",
			target: "chat",
			match: (event) => {
				const action = decodeRpMessageActionEvent(event);
				return openingActionMatch(event) === null && (action?.operation === "delete" || action?.operation === "reroll") ? {
					id: String(event.seq),
					role: "start"
				} : null;
			},
			...
			buildViewNode: (context) => context.state?.action === void 0 ? null : {
				...
				id: context.id,
				target: "chat",
				anchorSeq: context.state.seq + .09,
				location: context.start?.location ?? { kind: "unresolved" },
				visibility: "visible",
				data: context.state
			}
		...
		*/
		function RoleplayCommitToolView({ block }) {
			if (!("kind" in block)) return h("span", {
			...
			}, h("strong", null, "回复未能完成保存"), h("span", null, "正文仍会保留，但本次会话变量变化没有生效。请重新生成这条回复。"));
		}
		...
		function successfulCommitRows(host) {
			const rows = [];
		...
		function userActionMatch(event) {
			const target = decodeRpMessageActionEvent(event)?.targets.find((candidate) => candidate.kind === "message" && candidate.role === "user");
			if (target !== void 0) return {
				id: String(target.messageId),
				role: "update"
			};
			return event?.type === "user/message" && event.surfaceOp === "append" && event.data?.source?.kind === "user" ? {
				id: userMessageId(event),
				role: "start"
			} : null;
		}
		...
		function updateUserActionState(state, event) {
			const action = decodeRpMessageActionEvent(event);
			if (action !== void 0) return action.operation === "edit" ? {
				...state,
		...
		function UserFloorActions(props) {
			...
			return (location.kind === "turn" || location.kind === "step") && location.turn.status === "closed" ? h(FloorActions, {
				...props,
				turn: location.turn,
				surface: "user",
		...
		function AssistantFloorEffects(props) {
			...
			return h(react.default.Fragment, null, h(AssistantEffectNodeMarker), h(SettledAssistantTraceEffect, { target: props.node.data.target }), props.node.data.edited === true ? h(EditedMessagePortal, {
				surface: "assistant",
		...
		function OpeningFloorEffects(props) {
			...
			if (props.node.data.deleted === true) return h(DeletedOpeningTraceMarker, { target: props.node.data.target });
			return h(react.default.Fragment, null, h(AssistantEffectNodeMarker), h(SettledAssistantTraceEffect, { target: props.node.data.target }), props.node.data.edited === true ? h(EditedMessagePortal, {
				surface: "assistant",
				text: props.node.data.text
			}) : null);
		...
		function failedTurnStatus(matched, detail, diagnostic = null) {
			...
			if (kind === "blocked") return {
				...
				message: hasPartial ? recoveryMessage("已生成的内容可能不完整。", "已生成的内容可能不完整。") : recoveryMessage("没有生成新的回复。", "没有生成新的回复，可以重新尝试。")
			};
			return {
				state: "warning",
				title: "回复已中断",
		...
		function SettledAssistantTraceEffect({ target }) {
			const ref = (0, react.useRef)(null);
			const owner = target === void 0 ? "settled-assistant" : `settled-${rpMessageActionTargetKey(target)}`;
			(0, react.useLayoutEffect)(() => {
				...
				if (!(host instanceof HTMLElement)) return void 0;
				return ownDynamicRows(host, () => settledAssistantTraceRows(host), "data-rp-message-actions-hidden-trace", owner);
			}, [owner]);
			return h("span", {
				ref,
				className: css.traceEffectAnchor,
				hidden: true,
				"aria-hidden": true
			});
		}
		...
		function settledAssistantTraceRows(host) {
			const hidden = [];
			...
			for (let row = host?.previousElementSibling ?? null; row !== null; row = row.previousElementSibling) {
				...
				if (kind === "rp-message-avatar-assistant" || kind === "rp-message-avatar-opening" || kind === "rp-message-avatar-user" || kind === "rp-floor-user-actions") continue;
				hidden.push(row);
			}
			return hidden;
		}
		...
		function settledTurnTailRow(host) {
		...
		function DeletedUserTraceMarker({ target }) {
			...
			(0, react.useLayoutEffect)(() => {
				...
				if (!(host instanceof HTMLElement)) return void 0;
				return ownDynamicRows(host, () => deletedUserRows(host), "data-rp-message-actions-hidden-deleted-user", `deleted-user-${rpMessageActionTargetKey(target)}`);
			}, [target]);
			return h("span", {
				ref,
		...
		function SuffixActionEffect({ node, useSession, useChat }) {
			...
			(0, react.useLayoutEffect)(() => {
				...
				return ownDynamicRows(host, () => suffixActionRows(host, first, residentStartKey), "data-rp-message-actions-hidden-suffix", `suffix-${String(node.id)}`);
			}, [
				node.data.action,
				node.id,
				residentStartKey
			...
			return h("span", {
				ref,
				className: css.suffixEffectAnchor,
				hidden: true,
				"aria-hidden": true
			});
		}
		...
		*/
		function suffixResidentStartKey(chat, replacementStart, carrierAnchorSeq) {
			if (!Number.isSafeInteger(replacementStart) || typeof carrierAnchorSeq !== "number" || !Array.isArray(chat?.order) || typeof chat?.nodes?.get !== "function") return void 0;
			for (const key of chat.order) {
				const anchorSeq = chat.nodes.get(key)?.anchorSeq;
				if (typeof anchorSeq === "number" && anchorSeq >= replacementStart && anchorSeq < carrierAnchorSeq) return key;
			}
		}
		...
		function deletedAssistantTraceRows(host) {
		...
		function findFlowRow(host, key) {
			...
			for (let row = host?.previousElementSibling ?? null; row !== null; row = row.previousElementSibling) if (row.getAttribute?.("data-chat-flow-key") === key) return row;
			return null;
		}
		function ownDynamicRows(host, resolveRows, attribute, ownerSeed) {
			...
			const observer = typeof Observer === "function" ? new Observer(reconcile) : void 0;
		...
		function FloorActions({ sessionId, useSessions, turn, target, detail, edited = false, connection, sessions, surface = "assistant", actionLabel, copyText = "", forkSeq, messageTime, canEdit = true, failedAssistant = false, failureStatus = null, presetRecovery = null, nativeAssistant = false, userHadText = false, userHasNonTextContent = false }) {
			...
			const reroll = async (content) => {
				...
				setError(null);
				try {
					if ((await rpc(connection, "reroll", {
						sessionId,
						target,
						...content === void 0 ? {} : { content }
					})).sessionId !== sessionId) throw actionError("SESSION_CHANGED");
					setDialog(null);
					setEditing(false);
				} catch (reason) {
			...
			const edit = async (regenerate) => {
				...
				setPending(regenerate ? "save-reroll" : "edit");
				setError(null);
				try {
			...
			const branch = async (atSeq) => {
				...
				setError(null);
				try {
					const current = await rpc(connection, "get", {
						sessionId,
						target
					});
			...
			const rerollAction = detail?.canReroll === true ? h(FloorActionButton, {
				...
				ariaLabel: pending === "reroll" ? "正在重新生成回复" : userSurface ? `从第 ${actionTurn} 条消息重新生成` : `重新生成第 ${actionTurn} 条回复`,
			...
			const editAction = canEdit && detail?.canEdit === true ? h(FloorActionButton, {
				label: `编辑${unitLabel}`,
				ariaLabel: resolvedActionLabel === void 0 ? `编辑第 ${actionTurn} 条${unitLabel}` : `编辑${unitLabel}`,
			...
			const deleteAction = detail?.canDelete === true ? h(FloorActionButton, {
				label: `删除${unitLabel}`,
				ariaLabel: resolvedActionLabel === void 0 ? `删除第 ${actionTurn} 条${unitLabel}` : `删除${unitLabel}`,
			...
			}, detail?.canReroll === true ? h(_deepseek_ai_dsh_client_ui_primitives.Button, {
				variant: "primary",
				size: "sm",
			...
			return h(MotionConfig, {
				...
				features: domAnimation,
				strict: true
			}, edited && detail !== null && !editing ? h(EditedMessagePortal, {
				surface,
				text: copyText,
				userHadText
			}) : null, h("div", {
			...
			}) : nativeAssistant ? h(react.default.Fragment, null, h(NativeAssistantActionsVisibilityEffect), branchAction === null ? null : h(NativeAssistantBranchEffect), branchAction, rerollAction, editAction, deleteAction) : h("div", {
				className: css.floorActions,
				role: "toolbar",
				"aria-label": toolbarLabel
			}, userSurface && Number.isFinite(messageTime) ? h("span", { className: css.userFloorTime }, formatMessageTime(messageTime)) : null, visibleText !== "" ? h(CopyActionButton, {
				text: visibleText,
				turn: actionTurn,
				surface,
				unitLabel,
				named: resolvedActionLabel !== void 0
			}) : null, branchAction, rerollAction, editAction, deleteAction), error !== null && dialog === null && !editing ? h("div", {
				className: css.error,
				role: "alert"
			}, error) : null, h(RegenerateDialog, {
		...
		function NativeUserActionsEffect() {
			const ref = (0, react.useRef)(null);
			(0, react.useLayoutEffect)(() => {
				const message = messageRowForAction(actionNodeHost(ref.current), "user");
				if (!(message instanceof HTMLElement)) return void 0;
			...
			return h("span", {
				ref,
				className: css.nativeUserEffectAnchor,
				hidden: true,
				"aria-hidden": true
			});
		}
		function NativeAssistantBranchEffect() {
			...
			(0, react.useLayoutEffect)(() => {
				...
				if (!(branch instanceof HTMLElement)) return void 0;
				branch.setAttribute("data-rp-message-actions-hidden-native-branch", "");
				return () => {
					branch.removeAttribute("data-rp-message-actions-hidden-native-branch");
				};
			}, []);
			return h("span", {
				ref,
				className: css.nativeBranchEffectAnchor,
				hidden: true,
				"aria-hidden": true
			});
		}
		...
		function NativeAssistantActionsVisibilityEffect() {
			const ref = (0, react.useRef)(null);
			(0, react.useLayoutEffect)(() => {
				const actions = (ref.current?.closest?.("[data-slot=\"conversation.chat.assistant-actions\"]"))?.parentElement;
				if (!(actions instanceof HTMLElement)) return void 0;
				actions.setAttribute("data-rp-message-actions-assistant-visible", "");
				return () => {
					actions.removeAttribute("data-rp-message-actions-assistant-visible");
				};
			}, []);
			return h("span", {
				ref,
		...
		function nativeAssistantBranchButton(anchor) {
			const candidate = (anchor?.closest?.("[data-slot=\"conversation.chat.assistant-actions\"]"))?.nextElementSibling;
		...
		function EditedMessagePortal({ surface, text, userHadText = false }) {
			...
			(0, react.useLayoutEffect)(() => {
				const row = messageRowForAction(actionNodeHost(anchorRef.current), surface);
				...
				if (surface === "user") {
					...
					return () => {
						...
						if (original instanceof HTMLElement) if (previousAriaHidden === null) original.removeAttribute("aria-hidden");
						else original.setAttribute("aria-hidden", previousAriaHidden);
						setTarget(null);
					};
				}
				const original = assistantMessageContent(row);
				if (!(original instanceof HTMLElement)) return void 0;
			...
			return h(react.default.Fragment, null, h("span", {
				ref: anchorRef,
				className: css.editedMessagePortalAnchor,
				hidden: true,
				"aria-hidden": true
			}), target === null ? null : (0, react_dom.createPortal)(surface === "assistant" ? h("article", {
				className: css.editedAssistantMessage,
				"data-rp-message-actions-edited-body": ""
			}, h(_deepseek_ai_dsh_client_ui_primitives.MarkdownText, { text })) : h("div", {
				className: css.editedUserMessage,
				"data-rp-message-actions-edited-body": ""
			}, h(_deepseek_ai_dsh_client_ui_primitives.MessageText, { text })), target));
		}
		function InlineMessageEditorPortal({ surface, unitLabel, detail, body, setBody, pending, error, canSaveAndReroll, onCancel, onSave, onSaveAndReroll, userHadText = false, userHasNonTextContent = false }) {
			...
			const [target, setTarget] = (0, react.useState)(null);
			(0, react.useLayoutEffect)(() => {
				const messageRow = messageRowForAction(actionNodeHost(anchorRef.current), surface);
				if (!(messageRow instanceof HTMLElement)) return void 0;
				const portalTarget = surface === "user" ? userMessageContentStack(messageRow) : messageRow;
				if (!(portalTarget instanceof HTMLElement)) return void 0;
				messageRow.setAttribute("data-rp-message-actions-editing", "");
				const original = surface === "assistant" ? assistantMessageContent(messageRow) : userHadText ? portalTarget.lastElementChild : null;
			...
			return h(react.default.Fragment, null, h("span", {
				ref: anchorRef,
				className: css.inlineEditorPortalAnchor,
				"aria-hidden": true
			}), target === null ? null : (0, react_dom.createPortal)(h(InlineMessageEditor, {
				surface,
				unitLabel,
				detail,
				body,
		...
		function chatNodeRenderer(messageRow) {
			const renderer = (messageRow?.querySelector?.("[data-slot=\"conversation.chat.node\"]"))?.firstElementChild;
		...
		function messageRowForAction(host, surface) {
			const accepted = surface === "user" ? /* @__PURE__ */ new Set(["user", "steering"]) : /* @__PURE__ */ new Set(["assistant-step"]);
			for (let row = host?.previousElementSibling ?? null; row !== null; row = row.previousElementSibling) {
				const kind = row.dataset?.chatFlowKind;
		...
		function InlineMessageEditor({ surface, unitLabel, detail, body, setBody, pending, error, canSaveAndReroll, onCancel, onSave, onSaveAndReroll, userHasNonTextContent = false }) {
			const userSurface = surface === "user";
			...
			return h(m.div, {
				...
				transition: motionTransition
			}, h("label", null, h("span", { className: css.srOnly }, `编辑${unitLabel}`), h("textarea", {
				ref: textareaRef,
				value: body,
				...
				onKeyDown: keyDown
			})), userSurface && userHasNonTextContent ? h("p", { className: css.editorNotice }, "只修改文字，图片会保留。") : null, detail?.sharedAssetMutation === true ? h("p", { className: css.assetNotice }, "这次回复创建或修改过共享资料。编辑回复不会撤销这些资料变更。") : null, h(DialogError, { error }), h("div", { className: css.inlineEditorFooter }, h("div", null, h(m.button, {
				...actionGestures,
				type: "button",
				disabled: pending !== null,
				onClick: onCancel
				...
				...actionGestures,
				type: "button",
				"data-primary": showSaveAndReroll ? void 0 : "",
				disabled,
				onClick: onSave
			}, pending === "edit" ? "正在保存…" : "保存"), showSaveAndReroll ? h(m.button, {
				...actionGestures,
				type: "button",
				"data-primary": "",
				disabled,
				onClick: onSaveAndReroll
			}, pending === "save-reroll" || pending === "reroll" ? "正在保存并重新生成…" : "保存并重新生成") : null)));
		}
		function useAutoSizeTextarea(value) {
			const textareaRef = (0, react.useRef)(null);
			(0, react.useLayoutEffect)(() => {
				sizeInlineEditor(textareaRef.current);
			}, [value]);
			(0, react.useEffect)(() => {
		...
		function sizeInlineEditor(textarea) {
		...
		function formatMessageTime(time) {
			...
			const sameDay = value.getFullYear() === today.getFullYear() && value.getMonth() === today.getMonth() && value.getDate() === today.getDate();
			const clock = `${String(value.getHours()).padStart(2, "0")}:${String(value.getMinutes()).padStart(2, "0")}`;
			if (sameDay) return clock;
			return `${value.getFullYear() === today.getFullYear() ? `${value.getMonth() + 1}月${value.getDate()}日` : `${value.getFullYear()}年${value.getMonth() + 1}月${value.getDate()}日`} ${clock}`;
		}
		function CopyActionButton({ text, turn, surface, unitLabel, named = false }) {
			...
			return h(FloorActionButton, {
				...
				icon: copied ? _deepseek_ai_dsh_client_ui_primitives.IconCheckOutline16 : _deepseek_ai_dsh_client_ui_primitives.IconCopyOutline16,
				onClick: copy
			});
		}
		function FloorActionButton({ label, ariaLabel, icon: Icon, tone = "default", disabled = false, onClick }) {
			return h(_deepseek_ai_dsh_client_ui_primitives.Tooltip, {
				label,
				side: "bottom",
				delayMs: 350
			}, h(m.button, {
		...
		function RegenerateDialog({ open, close, surface, pending, error, failed = false, onConfirm }) {
			...
			return h(_deepseek_ai_dsh_client_ui_primitives.Modal, {
				...
				description: userSurface ? "会在当前对话中根据这条消息重新生成回复。" : failed ? "会移除当前未完成的内容，并根据刚才的消息重新生成。" : "会在当前对话中重新生成这条回复。",
		...
		function DeleteDialog({ open, close, surface, pending, error, failed = false, sharedAssetMutation = false, onConfirm }) {
			return h(_deepseek_ai_dsh_client_ui_primitives.Modal, {
				...
				onClose: close,
				closeLabel: "关闭删除确认",
				title: surface === "user" ? "删除这条消息及之后的内容？" : failed ? "删除这次未完成的回复及之后的内容？" : "删除这条回复及之后的内容？",
				description: failed ? "会保留触发这次生成的消息，并移除未完成的回复以及之后的全部对话。" : "会从选中的内容开始，移除当前对话中之后的全部消息与回复。",
				className: css.dialog,
				footer: h(react.default.Fragment, null, h(_deepseek_ai_dsh_client_ui_primitives.Button, {
					variant: "outline",
					autoFocus: true,
					...
					disabled: pending !== null,
					onClick: onConfirm
				}, pending === "delete" ? "正在删除…" : "确认删除"))
			}, h("div", { className: css.deleteSummary }, h("strong", null, failed ? "未完成回复之后的对话也会删除" : "选中内容之后的对话也会删除"), h("span", null, failed ? "触发这次生成的消息及之前的对话会保留；这次删除无法在当前界面撤销。" : "选中内容之前的对话会保留；这次删除无法在当前界面撤销。")), sharedAssetMutation ? h("p", { className: css.assetNotice }, "被移除的回复曾创建或修改共享资料。角色卡、世界书、人设、预设和文风不会随对话删除，也不会自动撤销。") : null, h(DialogError, { error }));
		}
		function DialogError({ error }) {
			return error === null ? null : h("div", {
				className: css.error,
				role: "alert"
			}, error);
		}
		...
		*/
		async function forkMessageBranch({ sessions, connection, sessionId, atSeq, target, replayEdit, content }) {
			const childId = await sessions.fork({
				sessionId,
				atSeq,
				increaseTitle: true
			});
			if (replayEdit) await rpc(connection, "edit", {
				sessionId: childId,
				target,
				content
			});
		...
		//#endregion
		exports.InlineMessageEditor = InlineMessageEditor;
		exports.actionNodeHost = actionNodeHost;
		exports.apply = apply;
		exports.assistantFloorNodeDefinition = assistantFloorNodeDefinition;
		...
		exports.nativeUserActions = nativeUserActions;
		exports.openingFloorNodeDefinition = openingFloorNodeDefinition;
		exports.settledAssistantTraceRows = settledAssistantTraceRows;
		exports.settledTurnTailRow = settledTurnTailRow;
		exports.sizeInlineEditor = sizeInlineEditor;
		exports.successfulCommitRows = successfulCommitRows;
