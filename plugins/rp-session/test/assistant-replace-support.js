 ...
 *
 * 与 `plugins/rp-message-actions/test/assistant-replace-support.js` 内容相同：仓库
 * 规定插件之间不得通过相对路径读取彼此的内部文件，这段探测又不足以抽成独立包，
 * 因此各自保留一份。
 */
...
function detectsAssistantReplaceSupport() {
  counter += 1
  const session = Session.create(SessionId(`rp-session-assistant-replace-probe-${counter}`))
